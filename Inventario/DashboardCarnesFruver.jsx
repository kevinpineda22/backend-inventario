...
import {
  faChartLine, faBoxesStacked, faListCheck, faWarehouse, faUsers, faBoxOpen,
  faFilter, faBroom, faBox, faCalendarDays, faFolderOpen, faTruckMoving,
  faIdCard, faTriangleExclamation, faCalendar, faArrowUp, faArrowDown, faMinus
...
const ENDPOINTS = {
  overview: `${API_BASE}/api/analytics/carnesyfruver/overview`,
  series: `${API_BASE}/api/analytics/carnesyfruver/series/daily`,
  topItems: `${API_BASE}/api/analytics/carnesyfruver/top/items`,
  byBodega: `${API_BASE}/api/analytics/carnesyfruver/by/bodega`,
  byOperario: `${API_BASE}/api/analytics/carnesyfruver/by/operario`,
};
...
function DashboardCarnesFruver() {
  ...
  const [kpis, setKpis] = useState({ inventarios: 0, totalRegistros: 0, totalCantidad: 0, operarios: 0, bodegas: 0 });
  const [seriePorDia, setSeriePorDia] = useState([]);
  const [topItems, setTopItems] = useState([]);
  const [porBodega, setPorBodega] = useState([]);
  const [porOperario, setPorOperario] = useState([]);
  ...
  const refOperarios = useRef(null);
  ...
  const refreshBodegasOpts = async () => {
    try {
      const base = ENDPOINTS.byBodega;
      if (!base) return setBodegasOpts([]);
  ...
  // Actualizar fechas cuando cambia la fuente
  useEffect(() => {
  ...
  const loadAlertas = async () => {
    try {
      ...
      const common = { from, to, categoria, bodega, consecutivo };
      const params = { ...common, operario };

      const urlTop = `${api.topItems}?${qs({ ...params, limit: 20 })}`;
  ...
  const load = async () => {
    ...
    setErr("");
    try {
      const params = { from, to, categoria, bodega, consecutivo, operario };
      ...
      const requests = [
        ...
        fetch(`${ENDPOINTS.byOperario}?${qs(params)}`).then((r) => r.json()),
      ];

      const [ov, se, ti, pb, po] = await Promise.all(requests);

      if (!ov.success) throw new Error(ov.message || "Error en overview");

      setKpis(ov.kpis || { inventarios: 0, totalRegistros: 0, totalCantidad: 0, operarios: 0, bodegas: 0 });
      setSeriePorDia(se.series || []);
      setTopItems(ti.top || []);
      setPorBodega(pb.by_bodega || []);
      setPorOperario(po.by_operario || []);

      await loadAlertas();
    } catch (e) {
      setErr(e.message);
      setKpis({ inventarios: 0, totalRegistros: 0, totalCantidad: 0, operarios: 0, bodegas: 0 });
      setSeriePorDia([]);
      setTopItems([]);
      setPorBodega([]);
      setPorOperario([]);
  ...
  const KPIs = () => (
    <div className="dashboard-inv-kpi-grid">
      <div className="dashboard-inv-kpi-card">
        ...
        <div className="dashboard-inv-kpi-title">Inventarios</div>
      ...
      <div className="dashboard-inv-kpi-card">
        ...
        <div className="dashboard-inv-kpi-title">Registros</div>
      ...
      <div className="dashboard-inv-kpi-card">
        ...
        <div className="dashboard-inv-kpi-title">Promedio por Registro</div>
      ...
      <div className="dashboard-inv-kpi-card">
        <FontAwesomeIcon icon={faUsers} className="dashboard-inv-kpi-icon" />
        <div className="dashboard-inv-kpi-title">Operarios</div>
        <div className="dashboard-inv-kpi-value">{kpis.operarios ?? 0}</div>
        {kpisAnterior && (
          <IndicadorCambio actual={kpis.operarios} anterior={kpisAnterior.operarios} />
        )}
      </div>
      <div className="dashboard-inv-kpi-card">
...
Código Principal: {codigo_barras}`
    ...
    const data = (topItems || []).map((d) => {
      // 🆕 PROCESAR CÓDIGOS ADICIONALES
      const codigosExtra = d.codigos_disponibles && d.codigos_disponibles.length > 1
        ? d.codigos_disponibles.slice(1).map(c => c.codigo_barras).join(", ")
        : "Ninguno";
      ...
      return {
        ...
        codigos_extra: codigosExtra // 🆕 NUEVOS CÓDIGOS ADICIONALES
      };
    });
  ...
  // ------ CHART: Barras horizontales (Análisis por operario) ------
  useEffect(() => {
    if (!refOperarios.current) return;
    
    const root = am5.Root.new(refOperarios.current);
    root.setThemes([am5themes_Animated.new(root)]);
    ...
    const series = chart.series.push(
      am5xy.ColumnSeries.new(root, {
        name: "Cantidad por Operario",
        xAxis,
        yAxis,
        valueXField: "cantidad",
        categoryYField: "operario",
        tooltip: am5.Tooltip.new(root, {
          labelText: `{operario}
...
Promedio: {promedio} por registro`
    ...
    const data = (porOperario || []).map((d) => ({
      operario: String(d.operario || "Sin operario").replace('@merka.com.co', ''),
      cantidad: Number(d.cantidad) || 0,
      registros: Number(d.registros) || 0,
      promedio: Number(d.promedio) || 0
    }));
    ...
    series.data.setAll(data);
    series.columns.template.setAll({ cornerRadiusTR: 4, cornerRadiusBR: 4 });
    
    // Colores alternativos
    series.columns.template.adapters.add(
      "fill",
      (_f, target) => {
        const index = target.dataItem.get("index");
        return index % 2 ? 
  ...
  const Filtros = React.useMemo(() => (
    <div className="dashboard-inv-filter-panel">
      <h3 className="dashboard-inv-panel-title">
        ...
        Filtros de Datos Carnes & Fruver
      </h3>
      ...
      <div className="dashboard-inv-filter-group">
        ...
        <div className="dashboard-inv-filter-item">
          ...
          <input
            list="dl-categorias-cf"
            placeholder="Todas"
            value={categoria}
            onChange={(e) => setCategoria(e.target.value)}
            className="dashboard-inv-input"
          />
          <datalist id="dl-categorias-cf">
        ...
        <div className="dashboard-inv-filter-item">
          <label className="dashboard-inv-label"><FontAwesomeIcon icon={faUsers} /> Operario</label>
          <input
  ...
  ), [from, to, categoria, bodega, consecutivo, operario, categoriasOpts, bodegasOpts]);
    ...
    <div className="dashboard-inv-container">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="dashboard-inv-header">
        <h2 className="dashboard-inv-main-title">
          <FontAwesomeIcon icon={faUsers} /> Dashboard Carnes & Fruver
        </h2>
      </motion.div>
      ...
      <div className="dashboard-inv-charts-grid">
        ...
        {/* Gráfico de análisis por operario */}
        {porOperario.length > 0 && (
          <div className="dashboard-inv-chart-panel">
            <h3 className="dashboard-inv-chart-title">
              <FontAwesomeIcon icon={faUsers} className="dashboard-inv-panel-icon" /> 
              Análisis por Operario
            </h3>
            <div ref={refOperarios} className="dashboard-inv-chart" />
          </div>
        )}
...
export default DashboardCarnesFruver;